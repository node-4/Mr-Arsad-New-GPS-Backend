...
exports.GetByIMEINumber = async(req, res) => {
    ...
    const result = await axios.get(`https://pullapi-s2.track360.co.in/api/v1/auth/pull_api?username=pullapi&password=Admin@1234&deviceImei=${data.imeiNumber}`);
    res.status(200).json({
        message:"Ok",
        result : result.data
    })
...
exports.GetAllDevices = async(req,res) => {
    try{
    const result = await axios.get("https://pullapi-s2.track360.co.in/api/v1/auth/pull_api?username=pullapi&password=Admin@1234");
    res.status(200).json({
        message: "ok",
        Devices : result.data
    })
