...
exports.sendOtp =  catchAsyncErrors(async(req,res) => {
    ...
    if(req.body.phone){
        ...
        if(!user){
          const otp  =   otpGenerator.generate(4, { alphabets: false, upperCase: false, specialChar: false })
            const data = {
                mobile: phone,
                otp: otp
            }
...
exports.SignIn = catchAsyncErrors( async(req,res) => {
    ...
    if(!user){
        return res.status(501).json({
            message: "Invalid or Email is not register "
        })
    }
    if(!user.password){
        return res.status(401).json({
            message: "Update Password before Login "
        })
    }
   const isPasswordValid = bcrypt.hashSync(req.body.password, user.password);
   if(!isPasswordValid){
